...
| `discover` | LoRD3 RDD scan / SuDDDS DiD discovery with the validation battery and honest effect estimates; `--plan` consumes an intake report. | `CSV` + `--treatment` (or `--plan intake_report.json`), `--design`, `--k`, `--q`, `--seed`, `--out` | `out/results.json` (plan mode: `out/discover_report.json`) |
