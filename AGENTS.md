...
| `survey` | One-command systematic design survey: run the dataset against ALL seven method families (rdd, did, kink, iv, sc, bunching, dee) with an applicability verdict per family — reasoned SKIPs included — and one visual report. Failed families are a recorded outcome, not a CLI failure. | `CSV`, `--context`, `--backend null\|agent\|anthropic\|gemini`, `--time`/`--unit`, `--cutoff COL=VALUE`/`--instrument COL`/`--threshold COL=VALUE` (repeatable), `--k`, `--q`, `--seed`, `--out` (default `out/survey`) | `OUT/survey.json`, `OUT/report.md` (always) + `OUT/report.html` (with `[report]`), `OUT/families/`, `OUT/intake/` |
